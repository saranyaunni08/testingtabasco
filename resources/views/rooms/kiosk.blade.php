...
<div class="modal fade" id="deleteModal{{ $room->id }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $room->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="deleteAuthForm{{ $room->id }}" action="{{ route('admin.rooms.destroy.Kiosk', ['roomId' => $room->id, 'buildingId' => $room->building_id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', function() {
      document.querySelectorAll('button[data-toggle="modal"]').forEach(button => {
        button.addEventListener('click', function() {
          ...
          const redirectUrl = `{{ route('admin.rooms.destroy.Kiosk', ['roomId' => '__ROOM_ID__', 'buildingId' => '__BUILDING_ID__']) }}`.replace('__ROOM_ID__', roomId).replace('__BUILDING_ID__', buildingId);
